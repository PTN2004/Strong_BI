...
class TestMySQLLoader:
    ...
    def test_is_schema_modifying_query(self):
        ...
        assert MySQLLoader.is_schema_modifying_query(
            "CREATE TABLE test (id INT)")[0] is True
        assert MySQLLoader.is_schema_modifying_query("DROP TABLE test")[
            0] is True
        assert MySQLLoader.is_schema_modifying_query(
        ...
        assert MySQLLoader.is_schema_modifying_query(
            "SELECT * FROM test")[0] is False
        assert MySQLLoader.is_schema_modifying_query(
            "INSERT INTO test VALUES (1)")[0] is False
        assert MySQLLoader.is_schema_modifying_query(
            "UPDATE test SET name = 'test'")[0] is False
        assert MySQLLoader.is_schema_modifying_query(
            "DELETE FROM test WHERE id = 1")[0] is False
    ...
    def test_connection_error(self, mock_connect):
        ...
        success, message = asyncio.run(
            _consume_loader(MySQLLoader.load(
                "test_prefix", "mysql://user:pass@host:3306/db"))
        )
