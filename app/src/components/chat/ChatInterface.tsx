...
const ChatInterface = ({ 
    ...
    <div className="loading-message-container px-6">
      <div className="flex gap-3 mb-6 items-start">
        <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
          <span className="text-primary-foreground text-xs font-bold">BI</span>
        </div>
  ...
  // Auto-send initial query on mount if database is available
  useEffect(() => {
    if (initialQuery && selectedGraph && !isProcessing && messages.length <= 1) {
  ...
  const handleSendMessage = async (query: string, version?: string) => {
    ...
    try {
      ...
      const reasoningMessageId = `reasoning-${Date.now()}`;
      let analysisInfo: {
        confidence?: number;
        missing?: string;
        ambiguities?: string;
      ...
      for await (const message of ChatService.streamQuery({
        ...
        if (message.type === 'status' || message.type === 'reasoning' || message.type === 'reasoning_step') {
          // Accumulate reasoning steps into a single 'ai-steps' message
          const stepText = message.content || message.message || '';
          
          setMessages(prev => {
            const existingIdx = prev.findIndex(m => m.id === reasoningMessageId);
            if (existingIdx >= 0) {
              const updated = [...prev];
              updated[existingIdx] = {
                ...updated[existingIdx],
          ...
          // Store chart configuration to display as an ECharts widget
          chartConfig = message.data || null;
          ...
          // Handle error
          const errorMsg = message.content || message.message || "An unknown error occurred.";
          toast({
            title: "Query Failed",
      ...
      // Add chart widget if available
      if (chartConfig) {
        const chartMessage: ChatMessageData = {
  ...
  return (
    <div className={cn("flex flex-col h-full bg-transparent", className)} data-testid="chat-interface">
      {/* Messages Area */}
      ...
      <div className="relative pt-6 pb-4 bg-gradient-to-t from-white via-white/95 dark:from-card dark:via-card/95 to-transparent z-10 shrink-0">
        <div className="px-6 mx-auto w-full max-w-4xl">
          ...
          {(selectedGraph?.id === 'DEMO_CRM' || selectedGraph?.name === 'DEMO_CRM') && (
            <div className="mb-4">
              <SuggestionCards
                ...
                disabled={isProcessing || disabled || (user?.role?.toLowerCase() === "viewer")}
              />
            </div>
          )}
          ...
          <QueryInput 
            ...
            placeholder={user?.role?.toLowerCase() === "viewer" ? "Bạn đang ở chế độ Chỉ xem (Viewer Mode)..." : "Ask me anything about your database..."}
            disabled={isProcessing || disabled || (user?.role?.toLowerCase() === "viewer")}
          />
          
          {/* Show loading indicator when processing */}
          {isProcessing && (
            <div className="flex items-center justify-center gap-2 mt-3 animate-fade-in" data-testid="processing-query-indicator">
              <LoadingSpinner size="sm" />
              <span className="text-muted-foreground text-xs font-medium">Processing your query...</span>
            </div>
          )}
          
          {/* Footer */}
          <div className="text-center mt-3">
            <p className="text-muted-foreground/60 text-[10px] uppercase tracking-wider font-semibold">
              Powered by <a href="https://falkordb.com" target="_blank" className="hover:text-primary transition-colors">FalkorDB</a>
            </p>
          </div>
        </div>
      </div>
