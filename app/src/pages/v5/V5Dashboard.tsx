...
const V5Dashboard = () => {
  ...
  return (
    <div className="w-full h-[calc(100vh-56px)] flex flex-col bg-[#fafafa] dark:bg-[#09090b] overflow-hidden relative font-sans">
      {/* Top Header */}
      <div className="w-full px-8 py-5 bg-white/80 dark:bg-black/80 backdrop-blur-md border-b shadow-sm z-10 shrink-0 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-700">StrongBI GenBI</h1>
        </div>
        {selectedGraph ? (
          <div className="flex items-center gap-3 text-sm px-4 py-1.5 rounded-full bg-white dark:bg-gray-800 border shadow-sm">
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
            <span className="text-muted-foreground font-medium">{selectedGraph.name}</span>
          </div>
        ) : (
          <div className="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 px-4 py-1.5 rounded-full border border-amber-200">
            <AlertCircle className="w-4 h-4" />
            <span className="font-medium">No database selected</span>
          </div>
        )}
      </div>
          ...
          {threads.map((thread) => (
            <div key={thread.id} className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex items-start gap-4 justify-end">
                <div className="pt-2">
                  <div className="bg-primary text-primary-foreground px-6 py-3 rounded-2xl rounded-tr-sm shadow-md text-[1.05rem] max-w-[80%] float-right">
                    {thread.query}
                  </div>
                </div>
              </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 px-4 pb-6 pt-12 bg-gradient-to-t from-white via-white/95 to-transparent dark:from-black dark:via-black/95 z-20">
        <div className="max-w-4xl mx-auto">
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="flex gap-3 relative shadow-2xl shadow-gray-200/50 dark:shadow-black/50 rounded-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 transition-all hover:shadow-3xl focus-within:ring-4 focus-within:ring-primary/20">
            <div className="relative flex-1 group">
              ...
              <Input
                ...
                className="pl-14 pr-6 py-8 text-[1.1rem] border-0 focus-visible:ring-0 focus-visible:ring-offset-0 bg-transparent rounded-l-full shadow-none"
                value={inputValue}
            ...
            <div className="p-2 shrink-0 flex items-center">
              <Button 
                ...
                className="h-12 w-12 p-0 rounded-full bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-600/90 text-white shadow-md transition-all active:scale-95 disabled:opacity-50"
              >
                {activeProcessing ? (
          ...
          <div className="text-center mt-4">
            <span className="text-[0.7rem] uppercase tracking-wider font-semibold text-gray-400 flex items-center justify-center gap-2">
