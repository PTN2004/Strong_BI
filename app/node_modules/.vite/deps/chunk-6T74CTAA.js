...
function axis(orient, scale2) {
  var tickArguments = [], tickValues = null, tickFormat = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k2 = orient === top || orient === left ? -1 : 1, x4 = orient === left || orient === right ? "x" : "y", transform = orient === top || orient === bottom ? translateX : translateY;
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format = tickFormat == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range2 = scale2.range(), range0 = +range2[0] + offset, range1 = +range2[range2.length - 1] + offset, position = (scale2.bandwidth ? center : number)(scale2.copy(), offset), selection = context.selection ? context.selection() : context, path2 = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
    ...
    text.attr(x4, k2 * spacing).text(format);
...
function treemap_default() {
  ...
  treemap.paddingInner = function(x4) {
    return arguments.length ? (paddingInner = typeof x4 === "function" ? x4 : constant_default(+x4), treemap) : paddingInner;
  };
  ...
  treemap.paddingTop = function(x4) {
    return arguments.length ? (paddingTop = typeof x4 === "function" ? x4 : constant_default(+x4), treemap) : paddingTop;
  };
  treemap.paddingRight = function(x4) {
    return arguments.length ? (paddingRight = typeof x4 === "function" ? x4 : constant_default(+x4), treemap) : paddingRight;
  };
  treemap.paddingBottom = function(x4) {
    return arguments.length ? (paddingBottom = typeof x4 === "function" ? x4 : constant_default(+x4), treemap) : paddingBottom;
  };
  treemap.paddingLeft = function(x4) {
    return arguments.length ? (paddingLeft = typeof x4 === "function" ? x4 : constant_default(+x4), treemap) : paddingLeft;
  };
  return treemap;
}
...
function cornerTangents(x02, y0, x12, y1, r1, rc, cw) {
  var x01 = x02 - x12, y01 = y0 - y1, lo = (cw ? rc : -rc) / sqrt(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x02 + ox, y11 = y0 + oy, x10 = x12 + ox, y10 = y1 + oy, x00 = (x11 + x10) / 2, y00 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D2 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt(max(0, r * r * d2 - D2 * D2)), cx0 = (D2 * dy - dx * d) / d2, cy0 = (-D2 * dx - dy * d) / d2, cx1 = (D2 * dy + dx * d) / d2, cy1 = (-D2 * dx + dy * d) / d2, dx0 = cx0 - x00, dy0 = cy0 - y00, dx1 = cx1 - x00, dy1 = cy1 - y00;
...
function point4(that, t0, t1) {
  ...
  that._context.bezierCurveTo(x02 + dx, y0 + dx * t0, x12 - dx, y1 - dx * t1, x12, y1);
}
...
function collide_default(radius) {
  ...
  function force() {
    ...
    function apply(quad, x02, y0, x12, y1) {
      ...
      return x02 > xi + r || x12 < xi - r || y0 > yi + r || y1 < yi - r;
    }
  }
...
function longitude(point5) {
  return abs4(point5[0]) <= pi4 ? point5[0] : sign2(point5[0]) * ((abs4(point5[0]) + pi4) % tau5 - pi4);
}
function polygonContains_default(polygon, point5) {
  var lambda = longitude(point5), phi2 = point5[1], sinPhi = sin3(phi2), normal = [sin3(lambda), -cos3(lambda), 0], angle = 0, winding = 0;
  var sum3 = new Adder();
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum3.add(atan22(k2 * sign3 * sin3(absDelta), cosPhi0 * cosPhi1 + k2 * cos3(absDelta)));
  ...
  return (angle < -epsilon6 || angle < epsilon6 && sum3 < -epsilon22) ^ winding & 1;
}
...
function azimuthalInvert(angle) {
  return function(x4, y4) {
    var z = sqrt2(x4 * x4 + y4 * y4), c = angle(z), sc = sin3(c), cc2 = cos3(c);
    return [
      atan22(x4 * sc, z * cc2),
      asin2(z && y4 * sc / z)
    ];
...
var uniform_default = (function sourceRandomUniform(source) {
  function randomUniform(min3, max5) {
    ...
    if (arguments.length === 1) max5 = min3, min3 = 0;
    else max5 -= min3;
    return function() {
...
var int_default = (function sourceRandomInt(source) {
  function randomInt(min3, max5) {
    if (arguments.length < 2) max5 = min3, min3 = 0;
...
var gamma_default = (function sourceRandomGamma(source) {
  ...
  function randomGamma(k2, theta) {
    ...
    var d = (k2 < 1 ? k2 + 1 : k2) - 1 / 3, c = 1 / (3 * Math.sqrt(d)), multiplier = k2 < 1 ? () => Math.pow(source(), 1 / k2) : () => 1;
    return function() {
      do {
        do {
          var x4 = randomNormal(), v2 = 1 + c * x4;
        } while (v2 <= 0);
        v2 *= v2 * v2;
        var u4 = 1 - source();
...
//# sourceMappingURL=chunk-6T74CTAA.js.map
