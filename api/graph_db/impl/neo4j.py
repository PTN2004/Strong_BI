...
class Neo4jGraphDatabase(GraphDatabase):
    ...
    async def query(self, query: str, params: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            async with self._driver.session(database=target_db) as session:
    ...
    async def clear_graph(self):
        try:
            ...
            logger.info(f"Neo4j: Cleared all nodes and relationships in database.")
        except Exception as e:
        ...
        if self.is_enterprise:
            try:
                async with self._driver.session(database="system") as session:
                    ...
                    graphs = [rec[0] for rec in record if rec[0] not in ("system")]
                    return graphs
