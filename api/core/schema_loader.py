...
async def _step_attempt_load(
    ...
    try:
        ...
        if success:
            ...
            logging.error("Database loader failed: %s",
                          str(result))  # nosemgrep
...
async def load_database(url: str, user_id: str, db=None):
    ...
    async def generate():
        ...
        try:
            ...
            logging.exception(
                "Unexpected error in connect_database stream: %s", str(e))
...
async def load_database_sync(url: str, user_id: str, db=None):
    ...
    if loader is None:
        raise InvalidArgumentError(
            "Invalid database URL format. Must be PostgreSQL, MySQL, or SQLite.")
