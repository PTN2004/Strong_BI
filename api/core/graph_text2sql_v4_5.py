...
from api.agents.business_insight_agent import BusinessInsightAgent
from api.agents.visual_chart_agent import VisualChartAgent
...
async def generate_insight_and_chart_v4_5(state: AgentState) -> dict:
    events = [
        {"type": "reasoning_step", "final_response": False, "message": "Bước 4.1: Chuyên gia Phân tích Kinh doanh trích xuất Insights..."},
        {"type": "reasoning_step", "final_response": False, "message": "Bước 4.2: Chuyên gia Đồ họa thiết kế Biểu đồ trực quan..."}
    ]
    ...
    try:
        ...
        # Run both agents concurrently in parallel threads
        insight_task = asyncio.to_thread(
        ...
        insight_res, chart_res = await asyncio.gather(insight_task, chart_task)

        insight = insight_res.get("insight") if insight_res else None
        chart_config = chart_res if chart_res and chart_res.get("should_visualize") else None

        if chart_config and chart_config.get("option"):
            events.append({
                "type": "chart_config",
                "data": chart_config.get("option"),
                "chart_type": chart_config.get("chart_type"),
                "final_response": False
            })

        usage_insight = insight_res.get("usage", {}) if insight_res else {}
        usage_chart = chart_res.get("usage", {}) if chart_res else {}

        total_p = state.get("total_prompt_tokens", 0) + usage_insight.get("prompt_tokens", 0) + usage_chart.get("prompt_tokens", 0)
        total_c = state.get("total_completion_tokens", 0) + usage_insight.get("completion_tokens", 0) + usage_chart.get("completion_tokens", 0)
        ...
        return {
            ...
            "api_calls_count": state.get("api_calls_count", 0) + 2
        }
    except Exception as e:
        logging.error(f"Insight/Chart Dual-Agent generation error: {e}")
        return {"events_to_yield": events}
...
def format_response(state: AgentState) -> dict:
    ...
    if state.get("direct_reply"):
        ...
        if num_rows == 0:
            ...
            ans = state.get("insight") or f"Đã tìm thấy {num_rows} bản ghi. (V4.5 Dual-Agent Pro)"
...
builder.add_node("insight_chart", generate_insight_and_chart_v4_5)
...
async def run_query_graph_v4_5(workspace_id: str, graph_id: str, chat_data: Any, db=None, user_id: str=None) -> AsyncGenerator[Union[dict, _Final], None]:
    initial_state = {
    ...
    try:
        ...
        save_ai_trace("LangGraph_Execution_V4_5", {"user_id": user_id, "graph_id": graph_id}, graph_trace)
